...
const handleBarcodeSubmit = async () => {
  ...
  if (!matched) {
    toast.error(t('transport_invoices_page.system_error_title'), { description: t('transport_invoices_page.barcode_not_found') })
    return
  }
...
const validate = (): boolean => {
  const errors = validateInvoiceDraft(draft.value, selectedRowsCount.value, t, 'transport_invoices_page')
...
const saveInvoice = async (mode: SaveMode) => {
  ...
  if (!validate()) {
    toast.error(t('transport_invoices_page.system_error_title'), { description: firstInvoiceValidationToastDescription(formErrors.value) })
    return
  }
  ...
  try {
    ...
    if (mode === 'close') {
      toast.success(t('transport_invoices_page.system_success_title'), { description: t('transport_invoices_page.system_save_success_body') })
      await navigateTo('/transport-invoices')
      return
    }
    if (mode === 'add') {
      toast.success(t('transport_invoices_page.system_success_title'), {
        description: `${t('transport_invoices_page.system_save_success_body')} ${t('transport_invoices_page.system_save_and_add_hint')}`,
      })
      invoicesStore.resetDraft()
      await navigateTo('/transport-invoices/create')
      return
    }
    toast.success(t('transport_invoices_page.system_success_title'), { description: t('transport_invoices_page.system_save_success_body') })
  }
    ...
    const msg = getErrorMessage(error) || t('transport_invoices_page.save_error')
    errorMessage.value = msg
    toast.error(t('transport_invoices_page.system_error_title'), { description: msg })
  }
  finally {
...
onMounted(async () => {
  ...
  try {
    ...
    if (!invoice) errorMessage.value = t('transport_invoices_page.not_found')
  }
  catch {
  ...
  <div class="mx-auto flex w-full flex-col gap-6 pb-10">
    <div class="flex min-w-0 items-start gap-3">
      ...
      <div class="min-w-0 space-y-1">
        <h1 class="text-2xl font-bold tracking-tight">{{ t('transport_invoices_page.edit_title') }}</h1>
        <p class="text-sm text-muted-foreground">{{ t('transport_invoices_page.edit_subtitle', { id }) }}</p>
      </div>
    </div>
    <div v-if="!canEditInvoice" class="rounded-xl border border-amber-200 bg-amber-50 px-4 py-10 text-center text-sm text-amber-800 dark:border-amber-800 dark:bg-amber-950/40 dark:text-amber-400">{{ t('transport_invoices_page.no_permission') }}</div>
    ...
    <template v-else>
      ...
      <Card class="gap-0 overflow-hidden py-0 shadow-sm">
        ...
        <CardContent class="space-y-6 px-4 py-5 sm:px-6 sm:py-6">
          <div class="grid gap-5 md:grid-cols-2">
            <div class="space-y-2"><label class="text-sm font-medium">{{ t('transport_invoices_page.reference_number') }}</label><Input :model-value="draft.reference_number || `#${id}`" disabled /></div>
            <div class="space-y-2">
              ...
              <Select :model-value="draft.warehouse_id ? String(draft.warehouse_id) : ''" @update:model-value="onWarehouseChange">
                ...
                ><SelectValue :placeholder="t('transport_invoices_page.select_warehouse')" /></SelectTrigger>
                <SelectContent>
            ...
            <div class="space-y-2"><label class="text-sm font-medium">{{ t('transport_invoices_page.customer_name') }}</label><Input v-model="draft.customer_name" /></div>
            <div class="space-y-2">
              <label class="text-sm font-medium">{{ t('transport_invoices_page.district') }}</label>
              <Select :model-value="draft.district_id ? String(draft.district_id) : ''" @update:model-value="onDistrictChange">
                <SelectTrigger class="w-full"><SelectValue :placeholder="t('transport_invoices_page.select_district')" /></SelectTrigger>
                <SelectContent>
            ...
            <div class="space-y-2">
              <label class="text-sm font-medium">{{ t('transport_invoices_page.customer_mobile') }}</label>
            ...
            <div class="space-y-2">
              <label class="text-sm font-medium">{{ t('transport_invoices_page.address') }}</label>
        ...
        <CardContent class="space-y-4 px-4 py-5 sm:px-6 sm:py-6">
          <div class="grid gap-3 md:grid-cols-2">
            <div class="space-y-2">
              ...
              <div class="relative"><Search class="pointer-events-none absolute start-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input v-model="productSearch" class="ps-9" :placeholder="t('transport_invoices_page.product_search_placeholder')" :disabled="!draft.warehouse_id" /></div>
            ...
            <div class="space-y-2">
              ...
              <div class="relative"><Barcode class="pointer-events-none absolute start-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input v-model="barcodeInput" class="ps-9" :placeholder="t('transport_invoices_page.barcode_placeholder')" :disabled="!draft.warehouse_id" @keydown.enter.prevent="handleBarcodeSubmit" /></div>
              <Button type="button" variant="outline" class="w-full gap-2 md:w-auto" :disabled="loadingProducts || !draft.warehouse_id" @click="showAllProducts"><Loader2 v-if="loadingProducts" class="size-4 animate-spin" /><span>{{ t('transport_invoices_page.products') }}</span></Button>
            </div>
          </div>
              ...
              <Table>
                <TableHeader class="hidden md:table-header-group">
                  <TableRow class="bg-muted/40 hover:bg-muted/40">
                    <TableHead class="min-w-[200px] text-start">{{ t('transport_invoices_page.col_product') }}</TableHead>
                    <TableHead class="min-w-[180px] text-start">{{ t('transport_invoices_page.row_description') }}</TableHead>
                    <TableHead class="w-24 text-start">{{ t('transport_invoices_page.qty') }}</TableHead>
                    <TableHead class="w-32 text-start">{{ t('transport_invoices_page.unit_price') }}</TableHead>
                    <TableHead class="w-44 text-start">{{ t('transport_invoices_page.discount_percent') }}</TableHead>
                    <TableHead class="w-32 text-start">{{ t('transport_invoices_page.row_total') }}</TableHead>
                    <TableHead class="w-12 text-start">{{ t('transport_invoices_page.col_actions') }}</TableHead>
                  </TableRow>
                  </TableHeader>
                <TableBody>
                  <TableRow
                  ...
                  >
                    <TableCell class="block py-1.5 md:table-cell md:min-w-[220px]">
                      <span class="block text-xs font-medium text-muted-foreground mb-1 md:hidden">
                        {{ t('transport_invoices_page.col_product') }}
                      </span>
                      <div class="space-y-2">
                        ...
                        <Select v-if="item.product?.variations.length" :model-value="item.variation_id ? String(item.variation_id) : ''" @update:model-value="value => invoicesStore.setRowVariation(idx, Number(value))">
                          <SelectTrigger><SelectValue :placeholder="t('transport_invoices_page.select_variation')" /></SelectTrigger>
                    ...
                    <TableCell class="block py-1.5 md:table-cell md:min-w-[120px] md:text-start md:rtl:text-end">
                      <span class="block text-xs font-medium text-muted-foreground mb-1 md:hidden">
                        {{ t('transport_invoices_page.unit_price') }}
                      </span>
                    ...
                    <TableCell class="block py-1.5 md:table-cell md:min-w-[220px] md:text-start md:rtl:text-end">
                      <span class="block text-xs font-medium text-muted-foreground mb-1 md:hidden">
                        {{ t('transport_invoices_page.discount_percent') }}
                      </span>
                      <Select :model-value="item.discount_mode" @update:model-value="value => invoicesStore.setRowDiscountMode(idx, (value as 'fixed' | 'percentage'))">
                        ...
                        <SelectContent><SelectItem value="fixed">{{ t('transport_invoices_page.discount_mode_fixed') }}</SelectItem><SelectItem value="percentage">{{ t('transport_invoices_page.discount_mode_percentage') }}</SelectItem></SelectContent>
                      </Select>
                    ...
                    <TableCell class="flex justify-between items-center py-1.5 md:table-cell md:text-start md:rtl:text-end md:font-medium md:tabular-nums">
                      <span class="text-xs font-medium text-muted-foreground md:hidden">
                        {{ t('transport_invoices_page.row_total') }}
                      </span>
          ...
          <div class="flex flex-wrap gap-2"><Button type="button" variant="outline" class="gap-1.5" @click="invoicesStore.addRow"><Plus class="size-4" />{{ t('transport_invoices_page.add_row') }}</Button><Button type="button" variant="outline" class="text-red-600" @click="clearDialogOpen = true">{{ t('transport_invoices_page.clear_all') }}</Button></div>
          <div class="grid gap-3 rounded-lg border bg-muted/20 p-4 sm:grid-cols-2 lg:grid-cols-5">
            <div><p class="text-xs text-muted-foreground">{{ t('transport_invoices_page.subtotal') }}</p><p class="mt-1 font-semibold tabular-nums">{{ formatMoney(summary.subtotal) }}</p></div>
            <div><p class="text-xs text-muted-foreground">{{ t('transport_invoices_page.total_discount') }}</p><p class="mt-1 font-semibold tabular-nums">{{ formatMoney(summary.totalDiscount) }}</p></div>
            <div class="space-y-1"><label class="text-xs text-muted-foreground">{{ t('transport_invoices_page.delivery_fees') }}</label><Input :model-value="draft.delivery_fees" type="number" min="0" class="tabular-nums" @update:model-value="value => draft.delivery_fees = Math.max(0, Number(value) || 0)" /></div>

              <div v-if="draft.other_fees > 0" class="space-y-1">
              <label class="text-xs text-muted-foreground">{{ t('transport_invoices_page.other_fees') }}</label>
              ...
              <div><p class="text-xs text-muted-foreground">{{ t('transport_invoices_page.grand_total') }}</p><p class="mt-1 text-lg font-bold tabular-nums">{{ formatMoney(summary.grandTotal) }}</p></div>
          </div>
        </CardContent>
      </Card>
      ...
      <div class="flex flex-col-reverse gap-3 rounded-xl border bg-card/80 px-4 py-4 shadow-sm sm:flex-row sm:items-center sm:justify-end sm:px-5">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger as-child><Button class="w-full gap-2 sm:w-auto sm:min-w-[190px]" :disabled="invoicesStore.submitting"><Loader2 v-if="invoicesStore.submitting" class="size-4 animate-spin" />{{ t('transport_invoices_page.save_options') }}</Button></DropdownMenuTrigger>
          <DropdownMenuContent align="end">
            <DropdownMenuItem @click="saveInvoice('close')">{{ t('transport_invoices_page.save_and_close') }}</DropdownMenuItem>
            <DropdownMenuItem @click="saveInvoice('add')">{{ t('transport_invoices_page.save_and_add') }}</DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      </div>
    </template>
    ...
    <AlertDialog :open="clearDialogOpen" @update:open="clearDialogOpen = $event">
      <AlertDialogContent><AlertDialogHeader><AlertDialogTitle>{{ t('transport_invoices_page.clear_all_title') }}</AlertDialogTitle><AlertDialogDescription>{{ t('transport_invoices_page.clear_all_body') }}</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>{{ t('common.cancel') }}</AlertDialogCancel><AlertDialogAction class="bg-red-600 text-white hover:bg-red-700" @click="invoicesStore.clearAllRows()">{{ t('transport_invoices_page.clear_all_confirm') }}</AlertDialogAction></AlertDialogFooter></AlertDialogContent>
    </AlertDialog>
  </div>
</template>
