  ...
  <div class="flex flex-col gap-6">
    <div class="flex items-center gap-3">
      <Button variant="ghost" size="icon" class="size-9 shrink-0" as-child>
        <NuxtLink to="/transport-invoices">
          <ArrowRight class="size-4" />
        </NuxtLink>
      </Button>
      <div class="min-w-0 space-y-1">
        <h1 class="text-2xl font-bold tracking-tight">{{ t('transport_invoices_page.create_title') }}</h1>
        <p class="text-sm text-muted-foreground">{{ t('transport_invoices_page.create_subtitle') }}</p>
      </div>
    </div>
    ...
    <template v-else>
      ...
      <Card class="gap-0 overflow-hidden py-0 shadow-sm">
        <div class="flex items-center gap-2 border-b bg-section-details border-section-details text-white px-4 py-3.5 sm:px-6">
          ...
          <h2 class="text-base font-semibold">{{ t('transport_invoices_page.details_section') }}</h2>
        </div>
        <CardContent class="space-y-6 px-4 py-5 sm:px-6 sm:py-6">
          <div class="grid gap-5 md:grid-cols-2">
            <div class="space-y-2">
              <label class="text-sm font-medium">{{ t('transport_invoices_page.reference_number') }}</label>
              <Input :model-value="draft.reference_number || t('transport_invoices_page.reference_generated_after_save')" disabled />
            </div>
            <div class="space-y-2">
              ...
              >
                ...
                >
                  <SelectValue :placeholder="t('transport_invoices_page.select_warehouse')" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem
            ...
            <div class="space-y-2">
              <label class="text-sm font-medium">{{ t('transport_invoices_page.customer_name') }}</label>
            ...
            <div class="space-y-2">
              ...
              >
                <SelectTrigger class="w-full">
                  <SelectValue :placeholder="t('transport_invoices_page.select_district')" />
                </SelectTrigger>
                <SelectContent>
            ...
            <div class="space-y-2">
              <label class="text-sm font-medium">{{ t('transport_invoices_page.invoice_date') }}</label>
              <Input v-model="draft.invoice_date" type="date" :aria-invalid="Boolean(formErrors.invoice_date)" />
      ...
      <Card class="gap-0 overflow-hidden py-0 shadow-sm">
        <div class="flex items-center gap-2 border-b bg-section-items border-section-items text-white px-4 py-3.5 sm:px-6">
          ...
          <h2 class="text-base font-semibold">{{ t('transport_invoices_page.items_section') }}</h2>
        </div>
        <CardContent class="space-y-4 px-4 py-5 sm:px-6 sm:py-6">
                  ...
                  >
                    <TableCell class="block py-1.5 md:table-cell md:align-top md:whitespace-normal md:min-w-[200px] md:py-3">
                      ...
                      <div class="flex min-w-0 flex-col gap-2">
                        ...
                        >
                          <SelectTrigger class="w-full max-w-full"><SelectValue :placeholder="t('transport_invoices_page.select_variation')" /></SelectTrigger>
                          <SelectContent>
                    ...
                    <TableCell class="block py-1.5 md:table-cell md:align-top md:py-3">
                      ...
                      <div class="w-full space-y-2 md:ms-auto md:max-w-44">
                        <Select :model-value="item.discount_mode" @update:model-value="value => invoicesStore.setRowDiscountMode(idx, (value as 'fixed' | 'percentage'))">
                          ...
                          <SelectContent>
                            <SelectItem value="fixed">{{ t('transport_invoices_page.discount_mode_fixed') }}</SelectItem>
                            <SelectItem value="percentage">{{ t('transport_invoices_page.discount_mode_percentage') }}</SelectItem>
                          </SelectContent>
                        </Select>
          ...
          <div class="flex flex-wrap gap-2">
            <Button type="button" variant="outline" class="gap-1.5" @click="invoicesStore.addRow">
              <Plus class="size-4" />
              {{ t('transport_invoices_page.add_row') }}
            </Button>
            <Button type="button" variant="outline" class="text-red-600" @click="clearDialogOpen = true">
              {{ t('transport_invoices_page.clear_all') }}
            </Button>
          </div>

          <div class="grid gap-3 rounded-lg border bg-muted/20 p-4 sm:grid-cols-2 lg:grid-cols-5">
            <div>
              <p class="text-xs text-muted-foreground">{{ t('transport_invoices_page.subtotal') }}</p>
              <p class="mt-1 font-semibold tabular-nums">{{ formatMoney(summary.subtotal) }}</p>
            </div>
            <div>
              <p class="text-xs text-muted-foreground">{{ t('transport_invoices_page.total_discount') }}</p>
              <p class="mt-1 font-semibold tabular-nums">{{ formatMoney(summary.totalDiscount) }}</p>
            </div>
            <div class="space-y-1">
              <label class="text-xs text-muted-foreground">{{ t('transport_invoices_page.delivery_fees') }}</label>
            ...
            <div>
              <p class="text-xs text-muted-foreground">{{ t('transport_invoices_page.grand_total') }}</p>
      ...
      <Card class="gap-0 overflow-hidden py-0 shadow-sm">
        <div class="border-b bg-section-terms border-section-terms text-Black px-4 py-3.5 sm:px-6"><h2 class="text-base font-semibold">{{ t('transport_invoices_page.terms_section') }}</h2></div>
        <CardContent class="px-4 py-5 sm:px-6 sm:py-6">
          <details class="rounded-lg border px-4 py-3">
            <summary class="cursor-pointer text-sm font-medium">{{ t('transport_invoices_page.terms_section') }}</summary>
            <textarea v-model="draft.terms" rows="4" class="mt-3 w-full resize-y rounded-md border border-input bg-background px-3 py-2 text-sm" :placeholder="t('transport_invoices_page.terms_placeholder')" />
          </details>
        </CardContent>
      </Card>

      <Card class="gap-0 overflow-hidden py-0 shadow-sm">
        <div class="border-b bg-section-notes border-section-notes text-Black px-4 py-3.5 sm:px-6"><h2 class="text-base font-semibold">{{ t('transport_invoices_page.notes_section') }}</h2></div>
        <CardContent class="px-4 py-5 sm:px-6 sm:py-6">
          <details class="rounded-lg border px-4 py-3">
            <summary class="cursor-pointer text-sm font-medium">{{ t('transport_invoices_page.notes_section') }}</summary>
            <textarea v-model="draft.notes" rows="4" class="mt-3 w-full resize-y rounded-md border border-input bg-background px-3 py-2 text-sm" :placeholder="t('transport_invoices_page.notes_placeholder')" />
          </details>
        </CardContent>
      </Card>
        ...
        <DropdownMenu>
          <DropdownMenuTrigger as-child>
            <Button class="w-full gap-2 sm:w-auto sm:min-w-[170px]" :disabled="invoicesStore.submitting">
              ...
              {{ t('transport_invoices_page.save_options') }}
            </Button>
          </DropdownMenuTrigger>
    ...
    <AlertDialog :open="clearDialogOpen" @update:open="clearDialogOpen = $event">
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>{{ t('transport_invoices_page.clear_all_title') }}</AlertDialogTitle>
          <AlertDialogDescription>{{ t('transport_invoices_page.clear_all_body') }}</AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
          <AlertDialogCancel>{{ t('common.cancel') }}</AlertDialogCancel>
          <AlertDialogAction class="bg-red-600 text-white hover:bg-red-700" @click="invoicesStore.clearAllRows()">
            {{ t('transport_invoices_page.clear_all_confirm') }}
          </AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
    </AlertDialog>
