...
const printFields = computed(() => {
  ...
  if (!current) return []
  const fields = [
    { label: t('transport_invoices_page.reference_number'), value: String(current.reference_number || `#${current.id}`) },
    { label: t('transport_invoices_page.shipment_status_label'), value: String(current.shipment_status_label || '—') },
    { label: t('transport_invoices_page.col_status'), value: statusLabel.value },
    { label: t('transport_invoices_page.customer_name'), value: String(current.customer_name || '—') },
    { label: t('transport_invoices_page.warehouse'), value: warehouseName.value },
    { label: t('transport_invoices_page.customer_mobile'), value: String(current.customer_mobile || '—') },
    { label: t('transport_invoices_page.district'), value: districtName.value || t('transport_invoices_page.district_unassigned') },
    { label: t('transport_invoices_page.address'), value: String(current.address || '—') },
    { label: t('transport_invoices_page.invoice_date'), value: fmtDate(current.invoice_date) },
    { label: t('transport_invoices_page.supply_date'), value: fmtDate(current.supply_date) },
    { label: t('transport_invoices_page.delivery_by'), value: deliveryByLabel.value },
  ]
  if (showDeliveryAgentFields.value) {
    fields.push(
      { label: t('transport_invoices_page.delivery_agent_name'), value: deliveryAgentName.value || '—' },
      { label: t('transport_invoices_page.delivery_agent_mobile'), value: deliveryAgentMobile.value || '—' },
    )
  }
  return fields
})
...
const printTotals = computed(() => {
  ...
  const totals: Array<{ label: string; value: string; emphasize?: boolean }> = [
    { label: t('transport_invoices_page.subtotal'), value: money(current.subtotal) },
    { label: t('transport_invoices_page.total_discount'), value: money(current.total_discount) },
  ]
  if (showDeliveryFee.value) {
    totals.push({ label: t('transport_invoices_page.delivery_fees'), value: money(deliveryFeeValue.value) })
  }
  if (showOtherFee.value) {
    totals.push({ label: t('transport_invoices_page.other_fees'), value: money(otherFeeValue.value) })
  }
  totals.push({ label: t('transport_invoices_page.grand_total'), value: grandTotal(current.grand_total), emphasize: true })
  return totals
})
...
const onPrintClick = async () => {
  ...
  try {
    ...
    current.status_label = t('transport_invoices_page.status_printed')
    toast.success(t('transport_invoices_page.print_marked_status_success'))
  }
  catch {
  ...
  <div class="flex flex-col gap-6">
    <div class="no-print flex items-center gap-3">
      ...
      <div v-if="invoice" class="flex items-center gap-2 shrink-0">
        ...
        <Button variant="outline" as-child><NuxtLink to="/transport-invoices">{{ t('common.close') }}</NuxtLink></Button>
      </div>
    </div>
    <div v-if="!canViewInvoices" class="no-print rounded-xl border border-amber-200 bg-amber-50 px-4 py-10 text-center text-sm text-amber-800 dark:border-amber-800 dark:bg-amber-950/40 dark:text-amber-400">{{ t('transport_invoices_page.no_permission') }}</div>
    ...
    <template v-else-if="invoice">
                  ...
                  <TableBody>
                    ...
                    >
                      <TableCell class="flex justify-between items-start gap-2 py-1.5 md:table-cell md:py-4">
                        <span class="text-xs font-medium text-muted-foreground md:hidden">{{ t('transport_invoices_page.col_product') }}</span>
                      ...
                      <TableCell class="flex justify-between items-start gap-2 py-1.5 md:table-cell md:py-4">
                        <span class="text-xs font-medium text-muted-foreground md:hidden">{{ t('transport_invoices_page.variation') }}</span>
                      ...
                      <TableCell class="flex justify-between items-start gap-2 py-1.5 md:table-cell md:py-4 md:text-end">
                        <span class="text-xs font-medium text-muted-foreground md:hidden">{{ t('transport_invoices_page.qty') }}</span>
                        <span class="tabular-nums">{{ asNumber(item.qty) }}</span>
                      </TableCell>
                      <TableCell class="flex justify-between items-start gap-2 py-1.5 md:table-cell md:py-4 md:text-end">
                        <span class="text-xs font-medium text-muted-foreground md:hidden">{{ t('transport_invoices_page.unit_price') }}</span>
                        <span class="tabular-nums">{{ money(item.unit_price) }}</span>
                      </TableCell>
                      <TableCell class="flex justify-between items-start gap-2 py-1.5 md:table-cell md:py-4 md:text-end">
                        <span class="text-xs font-medium text-muted-foreground md:hidden">{{ t('transport_invoices_page.discount_percentage') }}</span>
                        <span class="tabular-nums">{{ money(itemDiscountPercentage(item)) }}</span>
                      </TableCell>
                      <TableCell class="flex justify-between items-start gap-2 py-1.5 md:table-cell md:py-4 md:text-end">
                        <span class="text-xs font-medium text-muted-foreground md:hidden">{{ t('transport_invoices_page.line_discount') }}</span>
                        <span class="tabular-nums">{{ money(itemLineDiscount(item)) }}</span>
                      </TableCell>
                      <TableCell class="flex justify-between items-start gap-2 py-1.5 md:table-cell md:py-4 md:text-end">
                        <span class="text-xs font-medium text-muted-foreground md:hidden">{{ t('transport_invoices_page.row_total') }}</span>
      ...
      <SalesDocumentPrint
        :title="t('transport_invoices_page.view_title')"
        :reference="String(invoice.reference_number || `#${invoice.id}`)"
        :fields="printFields"
        :description-label="t('transport_invoices_page.invoice_description')"
        :description-html="asPrintHtml(invoice.description)"
        :items-label="t('transport_invoices_page.items_section')"
        :product-label="t('transport_invoices_page.col_product')"
        :variation-label="t('transport_invoices_page.variation')"
        :qty-label="t('transport_invoices_page.qty')"
        :unit-price-label="t('transport_invoices_page.unit_price')"
        :discount-percentage-label="t('transport_invoices_page.discount_percentage')"
        :line-discount-label="t('transport_invoices_page.line_discount')"
        :row-total-label="t('transport_invoices_page.row_total')"
        :items="printItems"
        :totals="printTotals"
        :terms-label="t('transport_invoices_page.terms_section')"
        :terms-html="asPrintHtml(invoice.terms)"
        :notes-label="t('transport_invoices_page.notes_section')"
