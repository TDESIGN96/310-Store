...
const saveInvoice = async (mode: SaveMode) => {
  ...
  if (!validate()) {
    toast.error(t('invoices_page.system_error_title'), { description: firstInvoiceValidationToastDescription(formErrors.value) })
    return
  }
  ...
  try {
    ...
    if (mode === 'add') {
      ...
      await navigateTo('/invoices/create')
      return
    }
    toast.success(t('invoices_page.system_success_title'), { description: t('invoices_page.system_save_success_body') })
  }
  ...
  <div class="mx-auto flex w-full flex-col gap-6 pb-10">
    <div class="flex min-w-0 items-start gap-3">
      ...
      <div class="min-w-0 space-y-1">
        <h1 class="text-2xl font-bold tracking-tight">{{ t('invoices_page.edit_title') }}</h1>
        <p class="text-sm text-muted-foreground">{{ t('invoices_page.edit_subtitle', { id }) }}</p>
      </div>
    </div>
    <div v-if="!canEditInvoice" class="rounded-xl border border-amber-200 bg-amber-50 px-4 py-10 text-center text-sm text-amber-800 dark:border-amber-800 dark:bg-amber-950/40 dark:text-amber-400">{{ t('invoices_page.no_permission') }}</div>
    ...
    <template v-else>
      ...
      <div class="flex flex-col-reverse gap-3 rounded-xl border bg-card/80 px-4 py-4 shadow-sm sm:flex-row sm:items-center sm:justify-end sm:px-5">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger as-child><Button class="w-full gap-2 sm:w-auto sm:min-w-[190px]" :disabled="invoicesStore.submitting"><Loader2 v-if="invoicesStore.submitting" class="size-4 animate-spin" />{{ t('invoices_page.save_options') }}</Button></DropdownMenuTrigger>
    ...
    <AlertDialog :open="clearDialogOpen" @update:open="clearDialogOpen = $event">
      <AlertDialogContent><AlertDialogHeader><AlertDialogTitle>{{ t('invoices_page.clear_all_title') }}</AlertDialogTitle><AlertDialogDescription>{{ t('invoices_page.clear_all_body') }}</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>{{ t('common.cancel') }}</AlertDialogCancel><AlertDialogAction class="bg-red-600 text-white hover:bg-red-700" @click="invoicesStore.clearAllRows()">{{ t('invoices_page.clear_all_confirm') }}</AlertDialogAction></AlertDialogFooter></AlertDialogContent>
    </AlertDialog>
  </div>
</template>
