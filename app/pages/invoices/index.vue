...
const pendingStatusChange = ref<{ row: InvoiceListItem, value: string } | null>(null)
...
const applyStatusChange = async (row: InvoiceListItem, value: string) => {
...
const applyBulkStatus = async (target: InvoiceStatusOption) => {
  ...
  try {
    ...
    toast.success(t('invoices_page.bulk_status_update_success', { count: rows.length }))
    selectedIds.value = new Set()
  }
  catch {
    toast.error(t('invoices_page.bulk_status_update_error'))
  }
  finally {
...
<template>
  <div class="flex flex-col gap-4">
      ...
      <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div class="flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center sm:gap-2 sm:flex-1">
          <div class="relative w-full sm:min-w-[200px] sm:max-w-sm">
            ...
            <Input v-model="search" :placeholder="t('invoices_page.list_search_placeholder')" class="h-9 pr-9 w-full" />
        ...
        <Table>
          <TableHeader class="hidden md:table-header-group">
            <TableRow class="bg-muted/40 hover:bg-muted/40">
              ...
              <TableHead class="rtl:text-start text-start font-medium whitespace-nowrap">{{ t('invoices_page.col_discount_amount') }}</TableHead>
              <TableHead class="rtl:text-start text-start font-medium whitespace-nowrap">{{ t('invoices_page.col_total') }}</TableHead>
          ...
          <TableBody>
            ...
            <TableRow v-for="row in sortedList" :key="row.id" class="flex flex-col gap-1 border-2 rounded-lg p-4 mb-4 shadow-sm md:table-row md:border md:border-b md:rounded-none md:p-0 md:mb-0 md:shadow-none hover:bg-muted/30 transition-colors align-middle cursor-pointer md:cursor-default" :class="{ 'bg-muted/20': selectedIds.has(row.id) }" v-bind="bindRow({ onLongPress: () => toggleSelect(row.id), onTap: () => (selectedCount > 0 ? toggleSelect(row.id) : navigateRow(`/invoices/show/${row.id}`)) })">
              ...
              <TableCell class="flex justify-between items-start gap-2 py-1.5 md:table-cell md:py-4">
                <span class="text-xs font-medium text-muted-foreground md:hidden">{{ t('invoices_page.col_supply_date') }}</span>
              ...
              <TableCell class="flex justify-between items-start gap-2 py-1.5 md:table-cell md:py-4" @click.stop>
    ...
    <AlertDialog :open="statusConfirmOpen" @update:open="onStatusConfirmOpenChange">
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle class="rtl:text-start">{{ t('invoices_page.status_backward_title') }}</AlertDialogTitle>
          <AlertDialogDescription class="rtl:text-start">
            {{ t('invoices_page.status_backward_body', { from: pendingStatusFromLabel, to: pendingStatusToLabel }) }}
    ...
    <AlertDialog :open="bulkStatusConfirmOpen" @update:open="v => { if (!v) cancelBulkStatusChange() }">
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle class="rtl:text-start">{{ t('invoices_page.bulk_status_backward_title') }}</AlertDialogTitle>
          <AlertDialogDescription class="rtl:text-start">
            {{ t('invoices_page.bulk_status_backward_body', { count: selectedCount, to: bulkStatusTarget ? statusOptionLabel(bulkStatusTarget) : '' }) }}
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
