...
onMounted(async () => {
  ...
  try {
    ...
    if (!loaded) {
      errorMessage.value = t('invoices_page.not_found')
      return
    }
  ...
  <div class="flex flex-col gap-6">
    <div class="flex items-center gap-3">
      <Button variant="ghost" size="icon" class="size-9 shrink-0" as-child>
        <NuxtLink to="/invoices"><ArrowRight class="size-4" /></NuxtLink>
      </Button>
      <div class="min-w-0 space-y-1">
        <h1 class="text-2xl font-bold tracking-tight">{{ t('invoices_page.return_title') }}</h1>
        <p class="text-sm text-muted-foreground">{{ t('invoices_page.return_subtitle', { id: invoiceId }) }}</p>
      </div>
    </div>
    ...
    <template v-else>
      ...
      <Card class="gap-0 overflow-hidden py-0 shadow-sm">
        ...
        <CardContent class="grid gap-4 px-4 py-5 sm:grid-cols-2 sm:px-6 sm:py-6">
          <div>
            <p class="text-xs text-muted-foreground">{{ t('invoices_page.return_original_invoice_ref') }}</p>
            <p class="text-sm font-medium">{{ originalReferenceNumber }}</p>
          </div>
          <div>
            <p class="text-xs text-muted-foreground">{{ t('invoices_page.return_generated_ref') }}</p>
            <Input :model-value="t('invoices_page.reference_generated_after_save')" disabled />
          </div>
          <div>
            <p class="text-xs text-muted-foreground">{{ t('invoices_page.customer_name') }}</p>
            <p class="text-sm">{{ String(invoice?.customer_name ?? '—') }}</p>
          </div>
          <div>
            <p class="text-xs text-muted-foreground">{{ t('invoices_page.warehouse') }}</p>
            <p class="text-sm">{{ warehouseName }}</p>
          </div>
          <div>
            <p class="text-xs text-muted-foreground">{{ t('invoices_page.invoice_date') }}</p>
            <p class="text-sm">{{ formatDisplayDate(invoice?.invoice_date) }}</p>
          </div>
          <div>
            <p class="text-xs text-muted-foreground">{{ t('invoices_page.supply_date') }}</p>
      ...
      <Card class="gap-0 overflow-hidden py-0 shadow-sm">
        ...
        <CardContent class="space-y-4 px-4 py-5 sm:px-6 sm:py-6">
              ...
              <Table>
                <TableHeader>
                  <TableRow class="bg-muted/40 hover:bg-muted/40">
                    ...
                    <TableHead class="text-end">{{ t('invoices_page.return_original_qty') }}</TableHead>
          ...
          <p class="text-xs text-muted-foreground">
            {{ t('invoices_page.return_qty_hint') }}
          </p>
        </CardContent>
      </Card>
      ...
      <Card class="gap-0 overflow-hidden py-0 shadow-sm">
        <div class="border-b bg-muted/40 px-4 py-3.5 sm:px-6">
          <h2 class="text-base font-semibold">{{ t('invoices_page.return_reason_label') }}</h2>
        </div>
