...
const addProductToRows = (product: QuotationProductOption, variationId: number | null) => {
  ...
  if (targetIndex === draft.value.items.length) {
    quotationsStore.addRow()
  }
...
const saveQuotation = async (mode: SaveMode) => {
  ...
  errorMessage.value = ''
  try {
    const payload = quotationsStore.buildPayload()
    ...
    const referenceNumber = typeof quotation?.reference_number === 'string' ? quotation.reference_number : ''
    if (referenceNumber) {
      quotationsStore.draft.reference_number = referenceNumber
    }
  ...
  <div class="flex flex-col gap-6">
    <div class="flex items-center gap-3">
      ...
      <div class="min-w-0 space-y-1">
        <h1 class="text-2xl font-bold tracking-tight">{{ t('quotations_page.create_title') }}</h1>
        <p class="text-sm text-muted-foreground">{{ t('quotations_page.create_subtitle') }}</p>
      </div>
    </div>
        ...
        <CardContent class="space-y-4 px-4 py-5 sm:px-6 sm:py-6">
            ...
            <div class="space-y-2">
              ...
              <div class="relative">
                ...
                <Input
                  ...
                  class="ps-9"
                  :placeholder="t('quotations_page.product_search_placeholder')"
                />
              </div>
                  ...
                  <TableRow v-for="(item, idx) in draft.items" :key="item.key">
                    ...
                    <TableCell class="align-top py-3">
                      <div class="ms-auto flex w-full max-w-24 flex-col items-end gap-1">
                        ...
                        />
                        <p v-if="formErrors[`row_${idx}_qty`]" class="w-full text-start text-xs text-red-600">
                          {{ formErrors[`row_${idx}_qty`] }}
                        </p>
                      </div>
                    </TableCell>
                    <TableCell class="align-top py-3">
                      <div class="ms-auto flex w-full max-w-32 flex-col items-end gap-1">
                        ...
                        />
                        <Button
                          v-if="item.unit_price_manual"
                          type="button"
                          variant="link"
                          class="h-auto self-end p-0 text-xs"
                          @click="quotationsStore.resetRowUnitPrice(idx)"
                        >
                          {{ t('common.reset') }}
                        </Button>
                        <p v-if="formErrors[`row_${idx}_unit_price`]" class="w-full text-end text-xs text-red-600">
                          {{ formErrors[`row_${idx}_unit_price`] }}
                        </p>
                      </div>
                    </TableCell>
                    <TableCell class="align-top py-3">
                    ...
                    <TableCell class="align-top py-3 text-start">
                      <div class="flex h-9 items-center justify-end">
                        <Button
                          ...
                          @click="quotationsStore.removeRow(idx)"
                        >
                          <Trash2 class="size-4" />
                        </Button>
                      </div>
          ...
          <div class="grid gap-3 rounded-lg border bg-muted/20 p-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div class="space-y-1">
              ...
              <Input
                :model-value="draft.delivery_fees"
                type="number"
                min="0"
                class="tabular-nums"
        ...
        <CardContent class="px-4 py-5 sm:px-6 sm:py-6">
          <details class="rounded-lg border px-4 py-3">
            ...
            <textarea
              v-model="draft.terms"
              rows="4"
              class="mt-3 w-full resize-y rounded-md border border-input bg-background px-3 py-2 text-sm"
              :placeholder="t('quotations_page.terms_placeholder')"
            />
          </details>
        </CardContent>
      </Card>
        ...
        <CardContent class="px-4 py-5 sm:px-6 sm:py-6">
          <details class="rounded-lg border px-4 py-3">
            ...
            <textarea
              ...
              rows="4"
              class="mt-3 w-full resize-y rounded-md border border-input bg-background px-3 py-2 text-sm"
              :placeholder="t('quotations_page.notes_placeholder')"
            />
          </details>
        </CardContent>
      </Card>
      ...
      <div class="flex flex-col-reverse gap-3 rounded-xl border bg-card/80 px-4 py-4 shadow-sm sm:flex-row sm:items-center sm:justify-end sm:px-5">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger as-child>
            <Button class="w-full gap-2 sm:w-auto sm:min-w-[170px]" :disabled="quotationsStore.submitting">
              <Loader2 v-if="quotationsStore.submitting" class="size-4 animate-spin" />
              {{ t('quotations_page.save_options') }}
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end">
            <DropdownMenuItem @click="saveQuotation('close')">
              {{ t('quotations_page.save_and_close') }}
            </DropdownMenuItem>
            <DropdownMenuItem @click="saveQuotation('add')">
              {{ t('quotations_page.save_and_add') }}
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      </div>
