...
const canDeletePurchaseBill = computed(() => canDelete('purchase_bills'))

const search = ref('')
const filterStatus = ref<'all' | 'pending' | 'in_delivery' | 'complete' | 'returned' | 'partially_returned'>('all')
const billDate = ref('')
const supplyDate = ref('')
const currentPage = ref(1)
const deleteTarget = ref<PurchaseBillListItem | null>(null)
...
const hasActiveFilters = computed(
  () => search.value.trim().length > 0 || filterStatus.value !== 'all' || Boolean(billDate.value) || Boolean(supplyDate.value),
)
...
const list = computed(() => purchaseBillsStore.list)
const sortedList = computed(() => {
  return [...list.value].sort((a, b) => {
    const aTime = new Date(a.bill_date).getTime()
    const bTime = new Date(b.bill_date).getTime()
    return bTime - aTime
  })
})
const pagination = computed(() => purchaseBillsStore.pagination)
const loading = computed(() => purchaseBillsStore.listLoading)
...
const loadRows = async (page = currentPage.value) => {
  ...
  await purchaseBillsStore.loadList(params)
...
const requestDelete = (row: PurchaseBillListItem) => {
...
const confirmDelete = async () => {
  ...
  deleting.value = true
  try {
    await purchaseBillsStore.deletePurchaseBill(deleteTarget.value.id)
    toast.success(t('purchase_bills_page.delete_success'))
    deleteDialogOpen.value = false
    deleteTarget.value = null
    await loadRows(currentPage.value)
  }
  catch {
    toast.error(t('purchase_bills_page.delete_error'))
  }
  finally {
...
const confirmBulkDelete = async () => {
  ...
  try {
    ...
    await Promise.all(ids.map(id => purchaseBillsStore.deletePurchaseBill(id)))
...
watch(
  [search, filterStatus, billDate, supplyDate],
  () => {
...
<template>
  <div class="flex flex-col gap-4">
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div>
        <h1 class="text-2xl font-bold tracking-tight">{{ t('purchase_bills_page.title') }}</h1>
        <p class="text-sm text-muted-foreground mt-1">{{ t('purchase_bills_page.subtitle_total', { count: pagination.total ?? sortedList.length }) }}</p>
      </div>
    </div>
    ...
    <template v-else>
        ...
        <div class="flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center sm:gap-2 sm:flex-1">
          ...
          <Select v-model="filterStatus">
            ...
            <SelectContent>
              ...
              <SelectItem value="in_delivery">{{ t('purchase_bills_page.status_in_delivery') }}</SelectItem>
              <SelectItem value="complete">{{ t('purchase_bills_page.status_complete') }}</SelectItem>
              <SelectItem value="returned">{{ t('purchase_bills_page.status_returned') }}</SelectItem>
      ...
      <div class="flex flex-col gap-4 rounded-lg border bg-card/30 p-4 sm:flex-row sm:flex-wrap sm:items-end">
        <div class="flex min-w-0 flex-1 flex-col gap-2 sm:min-w-[240px]">
          <span class="text-sm font-medium text-foreground">{{ t('purchase_bills_page.bill_date') }}</span>
          <div class="space-y-1">
            <label class="text-xs text-muted-foreground">{{ t('purchase_bills_page.bill_date') }}</label>
            <DatePickerInput v-model="billDate" class="w-full" />
          </div>
        </div>
        <div class="flex min-w-0 flex-1 flex-col gap-2 sm:min-w-[240px]">
          <span class="text-sm font-medium text-foreground">{{ t('purchase_bills_page.supply_date') }}</span>
          <div class="space-y-1">
            <label class="text-xs text-muted-foreground">{{ t('purchase_bills_page.supply_date') }}</label>
      ...
      <div class="rounded-lg border overflow-hidden">
        <Table>
          ...
          <TableBody>
            ...
            <TableRow v-for="row in sortedList" :key="row.id" class="flex flex-col gap-1 border-2 rounded-lg p-4 mb-4 shadow-sm md:table-row md:border md:border-b md:rounded-none md:p-0 md:mb-0 md:shadow-none hover:bg-muted/30 transition-colors align-middle cursor-pointer md:cursor-default" :class="{ 'bg-muted/20': selectedIds.has(row.id) }" v-bind="bindRow({ onLongPress: () => toggleSelect(row.id), onTap: () => (selectedCount > 0 ? toggleSelect(row.id) : navigateRow(`/purchase-bills/show/${row.id}`)) })">
              ...
              <TableCell class="flex justify-end gap-2 pt-3 border-t mt-2 md:table-cell md:border-0 md:pt-4 md:mt-0 md:text-end" @click.stop>
                <TableRowActions
                  :actions="[
                    ...
                    { key: `delete-${row.id}`, label: t('purchase_bills_page.action_delete'), type: 'button', icon: Trash2, tone: 'danger', disabled: !canDeletePurchaseBill, onClick: () => requestDelete(row) },
                  ]"
                  variant="invoice"
                  align="end"
                />
    ...
    <AlertDialog :open="deleteDialogOpen" @update:open="deleteDialogOpen = $event">
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>{{ t('purchase_bills_page.delete_title') }}</AlertDialogTitle>
          <AlertDialogDescription>{{ t('purchase_bills_page.delete_body') }}</AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
