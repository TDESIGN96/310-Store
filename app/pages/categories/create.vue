...
import { ArrowRight, Loader2, Check, ChevronDown, AlertCircle, Tag } from 'lucide-vue-next'
...
// ── Sync Validation (format/required checks only) ──────────────────────────────
...
// ── Uniqueness Check (called only at submit time) ──────────────────────────────
...
const createCategory = async () => {
  ...
  // Always validate parent against the latest category state from server.
  if (parentId.value !== null) {
    await loadParentCategories()
  }

  // Step 1: local format/required checks
  if (!validateLocal()) return

  submitting.value = true
  try {
    // Step 2: uniqueness checks (both names in parallel)
