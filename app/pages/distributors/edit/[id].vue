...
interface DistributorDetailResponse {
  ...
  data?: Record<string, unknown> | { distributor?: Record<string, unknown> }
}
...
const normalizeStatus = (value: unknown): 'active' | 'inactive' => {
  const statusText = getString(value).toLowerCase()
  if (statusText === 'active' || value === true || value === 1 || value === '1') return 'active'
  return 'inactive'
}
...
const loadDistributor = async () => {
  ...
  fetchError.value = ''
  try {
    const res = await $api<DistributorDetailResponse>(`/distributors/${distributorId.value}`)
    const rawData = res.data
    const data = (res.distributor
      ?? (isRecord(rawData) && 'distributor' in rawData ? rawData.distributor : undefined)
      ?? (isRecord(rawData) ? rawData : undefined)
      ?? null) as Record<string, unknown> | null

    if (!data) {
      fetchError.value = t('distributors_form.not_found')
      return
    }

    const admin = isRecord(data.admin) ? data.admin : null
    const user = isRecord(data.user) ? data.user : null

    nameEn.value = getString(data.name_en || data.distributor_name_en || data.name || data.company_name_en)
    nameAr.value = getString(data.name_ar || data.distributor_name_ar || data.company_name_ar)
    mobileNumber.value = getString(data.mobile || data.phone)
    email.value = getString(data.email)
    address.value = getString(data.address)
    location.value = getString(data.city || data.location)
    description.value = getString(data.description)
...
const validateForm = (): boolean => {
  ...
  if (!status.value.trim()) fieldErrors.value.status = t('distributors_form.validation_required')
  ...
  try {
    ...
    if (isValidationError(error)) {
      ...
      fieldErrors.value = {
        ...
        mobile_number: fe.mobile ?? fe.phone ?? '',
        email: fe.email ?? '',
        address: fe.address ?? '',
        location: fe.city ?? '',
        status: fe.status ?? '',
...
<template>
  <div class="flex flex-col gap-5">
    <div class="flex items-center gap-3">
      ...
      <div>
        <h1 class="text-2xl font-bold tracking-tight">{{ t('distributors_form.edit_title') }}</h1>
        <p class="text-sm text-muted-foreground mt-1">
          {{ t('distributors_form.edit_subtitle') }}
        </p>
      </div>
    </div>

    <div v-if="loadingDistributor" class="flex items-center justify-center gap-2 py-16 text-sm text-muted-foreground">
      <Loader2 class="size-5 animate-spin" />
      {{ t('distributors_form.loading') }}
    </div>
    ...
    >
      ...
      <p class="font-medium text-center">{{ fetchError }}</p>
    ...
    <div v-else class="rounded-lg border p-5 space-y-4">
      ...
      <div class="flex justify-end gap-2 pt-2">
        ...
        <Button type="button" class="bg-primary hover:bg-primary/90" :disabled="submitting" @click="updateDistributor">
