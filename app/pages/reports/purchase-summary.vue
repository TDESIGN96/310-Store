...
const loading = computed(() => reportsStore.purchaseSummaryLoading)
const generated = computed(() => reportsStore.purchaseSummaryGenerated)
const summary = computed(() => reportsStore.purchaseSummary)
const records = computed(() => reportsStore.purchaseSummaryRecords)
const pagination = computed(() => reportsStore.purchaseSummaryPagination)
...
const validateFilters = (): boolean => {
  ...
  if (!dateFrom.value.trim()) {
    fieldErrors.value.from_date = t('reports_purchase_summary.validation_from_required')
    valid = false
  }
  if (!dateTo.value.trim()) {
    fieldErrors.value.to_date = t('reports_purchase_summary.validation_to_required')
    valid = false
  }
  if (dateFrom.value.trim() && dateTo.value.trim() && isFromAfterToPickerDate(dateFrom.value, dateTo.value)) {
    fieldErrors.value.from_date = t('reports_purchase_summary.validation_from_after_to')
    valid = false
  }
  if (dateTo.value.trim() && isFuturePickerDate(dateTo.value)) {
    fieldErrors.value.to_date = t('reports_purchase_summary.validation_to_future')
    valid = false
  }
...
const generateReport = async (page = 1) => {
  ...
  try {
    await reportsStore.loadPurchaseSummary(params)
  }
  catch (error) {
...
const resetFilters = () => {
  ...
  fieldErrors.value = { from_date: '', to_date: '' }
  currentPage.value = 1
  reportsStore.resetPurchaseSummary()
}
...
<template>
  <div class="flex flex-col gap-6">
    ...
    >
      {{ t('reports_purchase_summary.no_permission') }}
    </div>
      ...
      <Card class="gap-0 overflow-hidden py-0 shadow-sm">
        ...
        <CardContent class="space-y-4 px-4 py-5 sm:px-6">
          <ReportDateRangeFilter
            ...
            :from-label="t('reports_purchase_summary.filter_from_date')"
            :to-label="t('reports_purchase_summary.filter_to_date')"
          ...
          <div class="flex flex-wrap gap-2">
            <Button
              ...
              @click="resetFilters"
            >
              {{ t('reports_purchase_summary.reset_filters') }}
            </Button>
            <Button
      ...
      <template v-if="generated">
        <Card class="gap-0 overflow-hidden py-0 shadow-sm">
          ...
          <CardContent class="grid items-center gap-4 px-4 py-5 sm:grid-cols-2 sm:px-6 lg:grid-cols-4">
            ...
            <div>
              <p class="text-xs text-muted-foreground">{{ t('reports_purchase_summary.summary_total_purchases') }}</p>
              <p class="text-lg font-semibold tabular-nums">{{ money(summary.total_purchases) }}</p>
            </div>
            <div>
              <p class="text-xs text-muted-foreground">{{ t('reports_purchase_summary.summary_average_purchase_order_value') }}</p>
              <p class="text-lg font-semibold tabular-nums">{{ money(summary.average_purchase_order_value) }}</p>
            </div>
            <div>
              <p class="text-xs text-muted-foreground">{{ t('reports_purchase_summary.summary_total_items_purchased') }}</p>
              <p class="text-lg font-semibold tabular-nums">{{ count(summary.total_items_purchased) }}</p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card class="gap-0 overflow-hidden py-0 shadow-sm">
          <div class="flex items-center gap-2 border-b bg-muted/40 px-4 py-3.5 sm:px-6">
            <h2 class="text-base font-semibold">{{ t('reports_purchase_summary.records_title') }}</h2>
          </div>
